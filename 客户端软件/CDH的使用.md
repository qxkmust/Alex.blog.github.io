...
```
步骤一：
	在三台虚拟机192.168.8.108-CDH-02、192.168.8.109-CDH-03、192.168.8.110-CDH-04分别安装Centos7系统，服务部署的规划：
	########################################################
...
步骤一：下载
	①下载CDH压缩包：
		登录CDH版本库http://archive.cloudera.com/cm5/cm/5/，下载对应linux系统的CDH版本，这里使用cloudera-manager-centos7-cm5.16.1_x86_64.tar.gz
	②非必须，下载离线库：
		登录CDH离线库http://archive.cloudera.com/cdh5/parcels/5.16.1/，下载对应的5.16.1版本的离线库
		CDH-5.16.1-1.cdh5.16.1.p0.3-el6.parcel
		CDH-5.16.1-1.cdh5.16.1.p0.3-el6.parcel.sha1
		manifest.json
步骤二：安装
	将cloudera-manager-centos7-cm5.16.1_x86_64.tar.gz上传到hadoop02、hadoop03、hadoop04下的/opt目录，并解压出两个文件夹cloudera和cm-5.16.1，将cloudera移动到cm-5.16.1目录中。以下是CDH5的目录介绍：
...
步骤三：配置
	①在三台主机创建名称为cloudera-scm的用户：
        useradd
        --system	//创建系统用户
        --home=/opt/cm-5.16.1/run/cloudera-scm-server	//指定home目录
        --no-create-home	//不使用默认的/home目录
        --shell=/bin/false  //用户不具有任何权限
        --comment "cloudera-scm" 	//用户描述
        cloudera-scm 	//自定义的用户名
     ②配置CM Agent的服务端地址
     	修改/opt/cm-5.16.1/etc/cloudera-scm-agent/config.ini:
     		server_host=hadoop02
     ③创建CM数据库
         /opt/cm-5.16.1/share/cmf/schema/scm_prepare_database.sh
         mysql	//使用mysql数据库
         cm		//数据库名称
     ...
     ⑤非必须，上传离线库文件到
     	将以下文件上传到/opt/cloudera/parcel-repo
            ...
            CDH-5.16.1-1.cdh5.16.1.p0.3-el6.parcel.sha1
            manifest.json
        注意：CM监控页也可以在线安装CDH-parcel，需要在代理中配置要下载的CDH版本地址：http://archive.cloudera.com/cdh5/parcels/5.16.1/
     ⑥创建parcels目录，并更改用户及用户组
     	mkdir /opt/cloudera/parcels
     	chown cloudera-scm:choudera-scm /opt/cloudera/parcels
	...
	②编写启动/关闭CM Agent服务脚本，创建/usr/local/bin/cm-agent.sh
        ...
        "start"){
                ...
                do
                        ...
                        ssh $i "/opt/cm-5.16.1/etc/init.d/cloudera-scm-agent start"
                done
        ...
        "stop"){
                ...
                do
                        ...
                        PID=$(ssh $i "ps -ef|grep cloudera-scm-agent|grep -v 'grep'|grep -v logfile|awk '{print \$2}'|xargs")
                        echo "PID=$PID"
                        if [ -n "$PID" ];then
                                ssh $i "kill -9 $PID"
                                ssh $i "/opt/cm-5.16.1/etc/init.d/cloudera-scm-agent restart"
                        fi
                done
        };;
        esac
...
步骤五：启动CM服务
	...
	②启动CM Agent服务
		cm-agent.sh start
步骤六：验证
	启动CM服务，等待大概3分钟，登录CM监控页面：http://192.168.8.108:7180（用户名和密码都是admin）
步骤七：集群部署
	在CM监控页面进行集群部署：
	-> 选择免费
	-> 继续
	-> 选定集群的主机
	-> 选择CDH的版本（默认）
	-> 等待下载安装（自动将.parcel文件和.sha1文件下载到/opt/cloudera/parcel-repo目录）
	...
	解决：①删除问题主机/opt/cm-5.16.1/lib/cloudera-scm-agent/cm_guid文件
		②单独重启问题主机上的CM Agent服务，/opt/cm-5.16.1/etc/init.d/cloudera-scm-agent restart
		③重新登录CM监控页，完成安装
```
